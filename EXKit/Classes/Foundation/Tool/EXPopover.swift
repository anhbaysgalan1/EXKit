...
public enum EXPopoverOption {
    ...
    case ignoreFromViewHeight(Bool)
}

@objc public enum EXPopoverType: Int {
    case up
    case down
    case left
    case right
...
open class EXPopover: UIView {
    ...
    open var popoverType: EXPopoverType = .down
    ...
    open var ignoreHeight: Bool = false
    ...
    public init(options: [EXPopoverOption]?, showHandler: (() -> ())? = nil, dismissHandler: (() -> ())? = nil) {
        super.init(frame: .zero)
        self.backgroundColor = .clear
        ...
        case .up:
            point = inView.convert(
                CGPoint(
                    ...
                    y: fromView.frame.origin.y - self.margin
                ), from: fromView.superview)
        case .down, .auto:
            point = inView.convert(
                CGPoint(
                    x: fromView.frame.origin.x + (fromView.frame.size.width / 2),
                    y: fromView.frame.origin.y + (ignoreHeight ? 0 : fromView.frame.size.height) + verticalOffset
                ), from: fromView.superview)
        case .left:
            point = inView.convert(
    ...
    open func show(_ contentView: UIView, point: CGPoint, inView: UIView) {
        if self.dismissOnBlackOverlayTap || self.showBlackOverlay {
            ...
            if self.dismissOnBlackOverlayTap {
                self.blackOverlay.addTarget(self, action: #selector(EXPopover.dismiss), for: .touchUpInside)
            }
        }
    ...
    @objc open func dismiss() {
        if self.superview != nil {
            if self.didDismissHandler == nil {
                self.willDismissHandlerDefault()
            }else {
    ...
    override open func draw(_ rect: CGRect) {
        ...
        switch self.popoverType {
        case .up:
            let bottomPoint = CGPoint(x: arrowPoint.x, y: self.bounds.height)
            let startPoint = CGPoint(x: arrowPoint.x - self.arrowSize.width * 0.25, y:bottomPoint.y - self.arrowSize.height * 0.5)
            let endPoint = CGPoint(x: arrowPoint.x + self.arrowSize.width * 0.25, y: startPoint.y)
            ...
            arrow.addQuadCurve(to: startPoint, controlPoint: bottomPoint)
        case .down, .auto:
            
            let startPoint = CGPoint(x: arrowPoint.x + self.arrowSize.width * 0.25, y: self.arrowSize.height * 0.5)
            let endPoint = CGPoint(x: arrowPoint.x - self.arrowSize.width * 0.25, y: self.arrowSize.height * 0.5)
            let topPoint = CGPoint(x: arrowPoint.x, y: 0)
//            arrow.move(to: CGPoint(x: arrowPoint.x, y: 0))
            ...
            //绘制曲线
            arrow.addQuadCurve(to: startPoint, controlPoint: topPoint)
            
        case .left:
            let orignPoint = CGPoint(x: self.bounds.width, y: self.bounds.height * 0.5)
            let startPoint = CGPoint(x:orignPoint.x - self.arrowSize.width * 0.25, y:orignPoint.y + self.arrowSize.height * 0.2)
            let endPoint = CGPoint(x: startPoint.x, y: orignPoint.y - self.arrowSize.height * 0.2)
            arrow.move(to: startPoint)
//            arrow.move(to: CGPoint(x: self.bounds.width, y: self.bounds.height * 0.5))
            ...
            arrow.addLine(to: endPoint)
            arrow.addQuadCurve(to: startPoint, controlPoint: orignPoint)
        case .right:
            let orignPoint = CGPoint(x: arrowPoint.x, y: self.bounds.height * 0.5)
            let startPoint = CGPoint(x:orignPoint.x + self.arrowSize.width * 0.25, y:orignPoint.y + self.arrowSize.height * 0.2)
            let endPoint = CGPoint(x: startPoint.x, y: orignPoint.y - self.arrowSize.height * 0.2)
            arrow.move(to: startPoint)
...
private extension EXPopover {
    
    func setOptions(_ options: [EXPopoverOption]?){
        if let options = options {
            for option in options {
                ...
                case let .ignoreFromViewHeight(value):
                    self.ignoreHeight = value
                case let .verticalOffset(value):
                    self.verticalOffset = value
                }
            }
        }
    }
    ...
    func create() {
        ...
        self.frame = frame
        print("pop == self.arrowShowPoint=\(self.arrowShowPoint)")
        let arrowPoint = self.containerView.convert(self.arrowShowPoint, to: self)
...
public extension EXPopover {
    
    static var commonTradePopOption:[EXPopoverOption] {
    ...
    static var upTradePopOption:[EXPopoverOption] {
        let options: [EXPopoverOption] = [.type(.up),
                                          .cornerRadius(4),
                                          .verticalOffset(5),
                                          .showBlackOverlay(true),
                                          .blackOverlayColor(UIColor.ThemeView.mask.withAlphaComponent(0.1)),
